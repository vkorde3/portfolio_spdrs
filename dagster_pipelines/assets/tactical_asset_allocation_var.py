...
def smart_download(tickers, start="2000-01-01", end=None):
    """
    Download multiple tickers and return merged Close DataFrame.
    """
    all_data = []
    for t in tickers:
...
# --- Download all tickers with caching ---
...
# --- VAR-based residual strategy (with look-ahead bias) ---
def var_strategy(residuals: pd.DataFrame, train_ratio=0.8, maxlags=5) -> pd.Series:
    n = len(residuals)
    ...
    forecast_df = pd.DataFrame(forecast, index=test.index, columns=residuals.columns)

    # trading signal = sign of forecast
    signals = np.sign(forecast_df)

    # shift signals forward so you trade on information available yesterday
    signals = signals.shift(1).reindex(test.index)

    # align and drop NaN after shifting
...
def backtest(strategy_returns: pd.Series) -> dict:
    ...
    cum_returns.plot(figsize=(12, 6), title="VAR-Based Residual Momentum Strategy Cumulative Returns")
    plt.ylabel("Cumulative Return")
    plt.xlabel("Date")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/strategy_performance_var.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    print("Running VAR strategy...")
    strategy_returns = var_strategy(residuals)
